    ...
    <Card component="main" role="main" aria-label="공격 경로 시각화" className="offensive-strategy-container" sx={{ width: '100%', height: 'calc(100vh - 120px)', bgcolor: 'background.paper', boxShadow: 3, m: 0 }}>
      <CardContent sx={{ p: 1, height: '100%', '&:last-child': { pb: 1 }, display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, gap: 1, overflow: 'hidden' }}>
        ...
        <Dialog
          ...
          maxWidth="md"
          fullWidth
          PaperProps={{ sx: { height: '70vh', maxHeight: '70vh', m: 0, position: 'relative', overflow: 'hidden', borderRadius: 2 } }}
        >
          <Box sx={{ p: 3, bgcolor: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 'bold' }}>📋 계산 로그</Typography>
              <IconButton
                onClick={() => setCalculationLogPopupOpen(false)}
          ...
          <DialogContent sx={{ p: 3, height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ fontFamily: 'monospace', whiteSpace: 'pre-wrap', fontSize: 12, lineHeight: 1.6, bgcolor: '#f5f5f5', p: 2, borderRadius: 1, height: '100%', overflowY: 'auto', border: '1px solid #ddd' }}>
              {typedLogText && typedLogText.length > 0 ? typedLogText : (
                <Typography variant="caption" className="empty-message">방책 리스트에서 경로를 선택하면 계산 과정을 표시합니다.</Typography>
              )}
            </Box>
          </DialogContent>
        </Dialog>
