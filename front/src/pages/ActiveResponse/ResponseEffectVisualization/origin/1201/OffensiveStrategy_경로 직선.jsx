...
function OffensiveStrategy({ deviceElementId, onSelectDevice }) {
  ...
  const parallelEdgesCanvasRef = useRef(null); // 평행선 그리기용 캔버스
  ...
  useEffect(() => {
    ...
    // 상위 5개 순위까지만 기본 표시 (0~4)
    const MAX_VISIBLE_RANKS = 5;

    // 모든 경로의 노드와 엣지 정보 수집 (상위 5개만)
    ...
    (pathList || []).forEach((path, pathIdx) => {
      ...
      // 상위 5개 순위만 기본 표시에 포함
      if (rank >= MAX_VISIBLE_RANKS) return;
    ...
    // Device ID 기반 엣지 맵 생성 (각 엣지에 대해 상위 5개 순위 정보만 저장)
    ...
    for (const [physEdgeId, rankInfo] of allPathEdges.entries()) {
      ...
      if (deviceFrom != null && deviceTo != null) {
        ...
        // 상위 5개 순위만 필터링
        const filteredRanks = [...rankInfo.ranks].filter(r => r < MAX_VISIBLE_RANKS);
        if (filteredRanks.length === 0) continue;
        
        const filteredLowestRank = Math.min(...filteredRanks);
        
        if (!devicePathEdges.has(deviceEdgeId)) {
          devicePathEdges.set(deviceEdgeId, { ranks: new Set(filteredRanks), lowestRank: filteredLowestRank, allRanks: [...filteredRanks] });
        } else {
          const existing = devicePathEdges.get(deviceEdgeId);
          filteredRanks.forEach(r => {
            existing.ranks.add(r);
            if (!existing.allRanks.includes(r)) {
              existing.allRanks.push(r);
            }
          });
          if (filteredLowestRank < existing.lowestRank) {
            existing.lowestRank = filteredLowestRank;
          }
        }
      }
    }
    ...
    // 선택된 경로의 순위 (5순위 초과 경로도 표시 가능)
    const selectedPathRank = selectedPath !== null ? (localSortedPathRanks.get(selectedPath) ?? selectedPath) : null;
    const isSelectedPathBeyondTop5 = selectedPathRank !== null && selectedPathRank >= MAX_VISIBLE_RANKS;
    ...
    (baseTopology.edges || []).forEach(e => {
      ...
      if (selectedPath !== null) {
        ...
        if (edgeRankInfo && edgeRankInfo.allRanks && edgeRankInfo.allRanks.length > 1) {
          // 여러 경로가 이 엣지를 공유하는 경우 - Canvas에서 평행선으로 그림
          const sortedRanks = [...edgeRankInfo.allRanks].sort((a, b) => a - b);
          
          // 기본 엣지는 투명하게 숨김 (Canvas에서 평행선으로 그릴 예정)
          edgeUpdates.push({
            id: e.id,
  ...
  // 5) Canvas에 평행선 그리기 (여러 경로가 같은 엣지를 공유할 때)
  useEffect(() => {
    if (!parallelEdgesCanvasRef.current || !topologyNetRef.current) return;
    
    const canvas = parallelEdgesCanvasRef.current;
    ...
    // 경로가 없거나 특정 경로가 선택된 경우 평행선 그리지 않음
    if (!pathList || pathList.length === 0 || selectedPath !== null) return;
    
    const baseTopology = initialTopologyRef.current ?? topologyData;
    
    // Physical ID -> Device 매핑
    const physicalToDeviceNodeId = new Map();
    const physicalToDeviceName = new Map();
    ...
    if (attackGraphData.nodes) {
      attackGraphData.nodes.forEach(n => {
        if (n.properties && n.properties.id) {
          const elemId = n.properties.id.startsWith('ml:') ? n.properties.id.slice(3) : n.properties.id;
          // elementId로 Device 노드 찾기
          const deviceNode = (baseTopology.nodes || []).find(dn => dn.elementId === elemId);
          if (deviceNode) {
            physicalToDeviceNodeId.set(n.id, deviceNode.id);
          }
        }
        const nodeName = n.label || n.properties?.name;
        if (nodeName) {
          physicalToDeviceName.set(n.id, nodeName);
          // name으로 Device 노드 찾기
          if (!physicalToDeviceNodeId.has(n.id)) {
            const deviceNode = (baseTopology.nodes || []).find(dn => dn.name === nodeName);
            if (deviceNode) {
              physicalToDeviceNodeId.set(n.id, deviceNode.id);
            }
          }
    ...
    const allRS = (attackGraphData.nodes || []).map(calcNodeRS).filter(v => typeof v === 'number');
    const networkRSMean = allRS.length ? allRS.reduce((a,b)=>a+b,0)/allRS.length : null;
    
    const pathMetricsLocal = (pathList || []).map((p, idx) => {
      const nodes = Array.isArray(p) ? p : [];
      const rsVals = nodes.map(node => {
        const full = (attackGraphData.nodes || []).find(n => n.id === node.id) || node;
        return calcNodeRS(full);
      ...
      const pathRS = rsVals.length ? rsVals.reduce((a,b)=>a+b,0)/rsVals.length : null;
      let success = null;
      if (pathRS != null && networkRSMean != null && networkRSMean > 0) {
        success = Math.min(Math.max(pathRS / networkRSMean, 0), 1);
      }
      return { index: idx, success };
    });
    
    const sortedPathRanksLocal = new Map();
    const sorted = [...pathMetricsLocal].sort((a, b) => (b.success ?? -1) - (a.success ?? -1));
    sorted.forEach((item, rank) => {
      sortedPathRanksLocal.set(item.index, rank);
    });
    
    // 상위 5개까지만 표시
    const MAX_VISIBLE_RANKS_CANVAS = 5;
    
    // 엣지별로 사용하는 경로들의 순위 수집 (상위 5개만)
    const edgePathRanks = new Map(); // edgeKey -> [rank1, rank2, ...]
    
    (pathList || []).forEach((path, pathIdx) => {
      const rank = sortedPathRanksLocal.get(pathIdx) ?? pathIdx;
      // 상위 5개만 포함
      if (rank >= MAX_VISIBLE_RANKS_CANVAS) return;
      
      const pathNodes = path || [];
      
      for (let i = 0; i < pathNodes.length - 1; i++) {
        const fromPhysId = pathNodes[i].id;
        const toPhysId = pathNodes[i + 1].id;
        const fromDeviceId = physicalToDeviceNodeId.get(fromPhysId);
        const toDeviceId = physicalToDeviceNodeId.get(toPhysId);
        
        if (fromDeviceId != null && toDeviceId != null) {
          const edgeKey = `${Math.min(fromDeviceId, toDeviceId)}-${Math.max(fromDeviceId, toDeviceId)}`;
          if (!edgePathRanks.has(edgeKey)) {
            edgePathRanks.set(edgeKey, []);
          }
          if (!edgePathRanks.get(edgeKey).includes(rank)) {
            edgePathRanks.get(edgeKey).push(rank);
          }
        }
      }
    });
    
    // 여러 경로가 공유하는 엣지만 평행선으로 그리기 (상위 5개만)
    edgePathRanks.forEach((ranks, edgeKey) => {
      if (ranks.length <= 1) return; // 단일 경로는 vis-network에서 처리
      
      const [fromId, toId] = edgeKey.split('-').map(Number);
      
      try {
        const fromPos = network.getPosition(fromId);
        const toPos = network.getPosition(toId);
        const fromScreen = network.canvasToDOM(fromPos);
        const toScreen = network.canvasToDOM(toPos);
        ...
        // 정렬된 순위 (상위 5개만)
        const sortedRanks = [...ranks].filter(r => r < MAX_VISIBLE_RANKS_CANVAS).sort((a, b) => a - b);
        if (sortedRanks.length === 0) return;
        
        const numLines = sortedRanks.length;
        const lineWidth = 4;
        const gap = 2;
        const totalWidth = numLines * lineWidth + (numLines - 1) * gap;
        const startOffset = -totalWidth / 2 + lineWidth / 2;
        
        sortedRanks.forEach((rank, i) => {
          const colorInfo = rankColors[Math.min(rank, rankColors.length - 1)];
          const offset = startOffset + i * (lineWidth + gap);
          
          const x1 = fromScreen.x + perpX * offset;
          const y1 = fromScreen.y + perpY * offset;
          const x2 = toScreen.x + perpX * offset;
          const y2 = toScreen.y + perpY * offset;
          ...
          ctx.moveTo(x1, y1);
          ctx.lineTo(x2, y2);
          ctx.strokeStyle = colorInfo.edge;
        ...
        // 노드 위치를 가져올 수 없는 경우 무시
      }
    });
    
  }, [pathList, attackGraphData, topologyData, selectedPath, rankColors]);

  // 네트워크 뷰 변경 시 평행선 다시 그리기
  useEffect(() => {
    if (!topologyNetRef.current) return;
    
    const redrawParallelEdges = () => {
      // 상태 변경을 트리거하여 평행선 다시 그리기
      if (parallelEdgesCanvasRef.current && topologyNetRef.current) {
        const canvas = parallelEdgesCanvasRef.current;
        const network = topologyNetRef.current;
        const ctx = canvas.getContext('2d');
        ...
        if (attackGraphData.nodes) {
          attackGraphData.nodes.forEach(n => {
            ...
            const nodeName = n.label || n.properties?.name;
            if (nodeName && !physicalToDeviceNodeId.has(n.id)) {
              const deviceNode = (baseTopology.nodes || []).find(dn => dn.name === nodeName);
              if (deviceNode) {
                physicalToDeviceNodeId.set(n.id, deviceNode.id);
              }
        ...
        // 상위 5개까지만 표시
        const MAX_VISIBLE_RANKS_REDRAW = 5;
        
        const edgePathRanks = new Map();
        
        (pathList || []).forEach((path, pathIdx) => {
          const rank = sortedPathRanksLocal.get(pathIdx) ?? pathIdx;
          // 상위 5개만 포함
          if (rank >= MAX_VISIBLE_RANKS_REDRAW) return;
        ...
        edgePathRanks.forEach((ranks, edgeKey) => {
          ...
          try {
            ...
            // 상위 5개만 필터링
            const sortedRanks = [...ranks].filter(r => r < MAX_VISIBLE_RANKS_REDRAW).sort((a, b) => a - b);
            if (sortedRanks.length === 0) return;
    ...
    topologyNetRef.current.on('zoom', redrawParallelEdges);
    topologyNetRef.current.on('dragEnd', redrawParallelEdges);
    
    return () => {
      if (topologyNetRef.current) {
        topologyNetRef.current.off('zoom', redrawParallelEdges);
        topologyNetRef.current.off('dragEnd', redrawParallelEdges);
      }
    };
  }, [pathList, attackGraphData, topologyData, selectedPath, rankColors]);

  // 6) Canvas에 파티클 애니메이션 그리기
  useEffect(() => {
