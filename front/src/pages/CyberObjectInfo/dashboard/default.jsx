...
export default function DashboardDefault() {
  ...
  const visualizations = [
    ...
    },
    {
      ...
      description: [
        { subTitle: '네트워크 데이터 융합', lines: [
          '콘솔 기반 수집·융합 파이프라인을 제공하며 Start/Stop/Restart 제어로 처리 상태와 데이터 흐름을 실시간 모니터링함.',
          '노드 시각화는 코어(흰색), 방화벽(적색), 라우터·레이어3(주황), 스위치(녹색), 서버·호스트/워크스테이션(파랑), 허브(청록)로 구분됨.'
        ]},
        { subTitle: '외부망 기본맵 가시화', lines: [
          '외부 연계 토폴로지를 3D로 구성하고 영역 필터·링크 유형 전환(물리/논리)·노드 선택 이벤트 로그를 통해 연결 구조를 분석함.',
          '링크는 실선(물리)·점선(논리)으로 표시되어 외부망 경로와 상호연결을 명확히 파악할 수 있음.'
        ]},
        { subTitle: '내부망 네트워크 토폴로지 가시화', lines: [
          '장비 유형별 필터(방화벽·라우터·스위치·서버·호스트/워크스테이션·허브)와 상세 오버레이를 통해 내부 자산 관계를 정밀 추적함.',
          '세 하위메뉴를 연계해 수집 데이터 검증 → 외부망 경로 파악 → 내부 확산(Lateral Movement) 예측까지 이어지는 분석 체계를 구성하고 대응 우선순위를 도출함.'
        ]},
      ],
      flowSteps: ['네트워크 데이터 수집·융합', '수집 데이터 시각적 분류', '외부망 기본맵 가시화', '내부망 토폴로지 가시화', '통합 분석 및 대응 우선순위 도출']
    },
    {
      id: 'target',
      ...
      description: [
        {
          subTitle: '네트워크 구조 분석 모듈',
          lines: [
            '수집한 노드·엣지 데이터를 기반으로 자산(표적) 간 연결성, 영향도, 의존 관계를 분석해 표적을 도출하도록 설계됨.',
            '표적의 연결 수(degree), 도달성/연결성 지표(con), 엣지 관계를 함께 해석해 네트워크 내부 파급 경로와 핵심 허브를 식별하고 전략적 차단 우선순위 표적을 산출함.'
          ]
        },
        {
          subTitle: '후보/핵심 표적 상세 가시화',
          lines: [
            '표적 간 연결 수와 고위험 군집(고연결·고도달)을 함께 추적해 표적 우선순위 근거를 정량화함.',
            '표적 데이터 기반 필터 조건, 그래프 상호작용, 통계 지표, 이벤트 로그가 동기화되어 지휘관이 단일 화면에서 탐지·해석·우선조치 결정을 연속 수행할 수 있음.',
            '선정된 핵심 표적은 후속 대응인 노드 분석 및 능동 대응책 생성 가시화기와 연계되어 표적 공격 방책 분석과 경로 생성에 활용됨.'
          ]
        }
      ],
    ...
    },
    {
      ...
      description: [
        {
          subTitle: '위험 노출도 및 공격 가능도 측정기',
          lines: [
            '내부망 토폴로지와 위협 지표를 기반으로 노드·경로 단위 위험도를 정량화하고 공격 확산 가능 구간과 병목 지점을 식별하도록 설계됨.',
            'RS(연결 정도 * 0.6 + 도달 정도 * 0.4) 기반 고·중·저 위험군 분포를 추적해 방책별 공격 효과와 잔존 위험 편차를 정량적으로 평가함.'
          ]
        },
        {
          subTitle: '능동 대응책 대응 효과/경로 가시화',
          lines: [
            '공격 경로(빨간 점선)와 정상 연결(보라 실선)을 분리 표시하고 정책 적용 전·후 변화 경로를 비교해 통제 효과를 직관적으로 검증하도록 구성됨.',
            '장치 유형 분포와 RS 구간(10% 단위) 분포를 함께 분석해 방책 경로 우선순위, 경로 표적 정보, 방책 계산 로그를 도출함.'
          ]
        }
      ],
                      ...
                      if (typeof desc === 'object') {
                        return (
                          <Box key={index} sx={{ display: 'flex', flexDirection: 'column', gap: 0.2, mt: index > 0 ? 1.5 : 0 }}>
                            <Typography color="#000" sx={{ lineHeight: 1.35, fontSize: '1.22em', fontWeight: 800 }}>
                              • {desc.subTitle}
                            </Typography>
