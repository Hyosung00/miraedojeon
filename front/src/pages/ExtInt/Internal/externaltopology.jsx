...
function NetworkTopology3D_LeftSidebar({ activeView = "default", onInspectorChange }) {
  ...
  useEffect(() => { 
    // Zone 7(내부망) 제외 나머지 전체 기본 선택
    if (allZones.length > 0) {
      setSelectedZones(prev => {
        const without7 = allZones.filter(z => z !== 7);
        if (prev.length === without7.length && without7.every(z => prev.includes(z))) return prev;
        return without7;
      });
    }
  }, [allZones]);
  ...
  return (
    <Card sx={{ width: '100%', height: 'calc(100vh - 132px)', bgcolor: 'background.paper', borderRadius: 2, boxShadow: 3, overflow: 'hidden' }}>
